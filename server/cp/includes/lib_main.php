...
function assign_dynamic($tmp) {
	$sql = 'SELECT id, number, type FROM ' . $GLOBALS['ecs'] -> table('template') . " WHERE filename = '$tmp' AND type > 0 AND remarks ='' AND theme='" . $GLOBALS['_CFG']['template'] . "'";
...
function set_affiliate() {
	...
	if (!empty($_GET['u']) && $config['on'] == 1) {
		if (!empty($config['config']['expire'])) {
			...
			setcookie('ecshop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24);
			// 过期时间为 1 天
		}
	}
}
...
function getLastOrderByUser($uid) {
	global $db, $ecs;
	$res = $db -> getRow("select car_id,start_time,end_time from " . $ecs -> table("car_order") . " where user_id='$uid' order by order_id limit 1");
	return $res;
}
?>
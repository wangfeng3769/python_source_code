...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('order_action') .
            ...
            'FROM ' . $GLOBALS['ecs']->table('car_order') . " WHERE order_no = '$order_sn'";
...
function getRegionTree( $region_id, $pArr=array() )
{
	global $db, $ecs;
	if(sizeof($region_id)>0)
	{
		array_push( $pArr, $region_id );
	}
	$parent_id = $db->getOne("select parent_id from ".$ecs->table("region")." where region_id=".$region_id) ;

	if($parent_id)
	{
		array_unshift($pArr, $parent_id) ;
		return getRegionTree( $parent_id, $pArr ) ;
	}
	else
	{
		return $pArr ;

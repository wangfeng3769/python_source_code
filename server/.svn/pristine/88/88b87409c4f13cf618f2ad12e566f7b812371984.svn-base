...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if ($row)
    {
        ...
        // 清除购物车中过期的数据
        clear_cart();
		$uri = preg_replace("/cp\/admin.+$/", "", $_SERVER['REQUEST_URI']);
		$rootpath = "http://".$_SERVER['HTTP_HOST'].$uri ;
		
		//$s = file_get_contents($rootpath."/index.php?app=home&mod=Public&act=doAdminLogin&email=admin@admin.com&password=chinamilk");
		//die($s);

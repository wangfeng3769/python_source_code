...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /*处理图片*/
    $img_name = basename($image->upload_image($_FILES['icon'],'bonus_icon'));
    $icon_url = $image->data_dir.'/'.'bonus_icon/'.$img_name;

    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (icon,type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount)
    VALUES ('$icon_url',
            '$type_name',
            '$_POST[type_money]',
            '$send_startdate',
...
function get_type_list()
{
    ...
    $sql = "SELECT b.bonus_type_id, COUNT(1) AS sent_count".
            ...
            " WHERE t.send_type=".SEND_BY_USER.

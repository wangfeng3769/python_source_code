...
if ($_REQUEST['act'] == 'order_query')
{
    ...
    $smarty->assign('action_link', array('href' => 'car_order.php?act=list', 'text' => $_LANG['02_order_list']));

    /* 显示模板 */
    assign_query_info();
    $smarty->display('car_order_query.htm');
}
/*------------------------------------------------------ */
//-- 配置订单原则
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'config')
{
    if($_POST)
    {
        ...
        {
            if(in_array($k, array('order_interval','order_min_time','order_max_time','order_book_time')))
            {
        ...
        $link[0]['href'] = 'car_order.php?act=config';
    ...
    else
    {
        ...
        $res = $db->getAll("select * from ".$ecs->table("shop_config")." where code in ('order_interval','order_min_time','order_max_time','order_book_time')");
    
        foreach($res as $k=>$v)
        {
            $smarty->assign($v['code'],$v['value']);
        }
        $smarty->display('order_config.htm');
    }
}
/*------------------------------------------------------ */
//-- 订单列表
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href' => 'car_order.php?act=order_query', 'text' => $_LANG['03_order_query']));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('car_order_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('car_order_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    if($order['car_id']>0)
    {
        $car_info = get_car_list($order['car_id']);
        $smarty->assign('car',$car_info);
    }
    ...
    else
    {
        ...
        $smarty->assign('action_link', array('href' => 'car_order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['02_order_list']));

        /* 显示模板 */
        assign_query_info();
        $smarty->display('car_order_info.htm');
    }
}
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    $smarty->assign('action_link', array('href' => 'car_order.php?act=delivery_list&' . list_link_postfix(), 'text' => $_LANG['09_delivery_order']));
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if(!empty($delivery_stock_result))
    {
        foreach ($delivery_stock_result as $value)
        {
            ...
            {
                /* 操作失败 */
                $links[] = array('text' => $_LANG['order_info'], 'href' => 'car_order.php?act=delivery_info&delivery_id=' . $delivery_id);
    ...
    {
        /* 操作失败 */
        $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'car_order.php?act=delivery_info&delivery_id=' . $delivery_id);
    ...
    /* 操作成功 */
    $links[] = array('text' => $_LANG['09_delivery_order'], 'href' => 'car_order.php?act=delivery_list');
    $links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'car_order.php?act=delivery_info&delivery_id=' . $delivery_id);
...
elseif ($_REQUEST['act'] == 'back_info')
{
    ...
    $smarty->assign('action_link', array('href' => 'car_order.php?act=back_list&' . list_link_postfix(), 'text' => $_LANG['10_back_order']));
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    if ('user' == $step)
    {
        ...
        /* 下一步 */
        ecs_header("Location: car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
        exit;
    }
    /* 编辑商品信息 */
       ...
       $res = $db->query($sql);
        ...
        if (is_spec($goods_attr) && !empty($prod))
        {
            if (!empty($goods_attr))
            {
                ...
                {
                    $url = "car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods";
    ...
    elseif ('goods' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            ecs_header("Location: car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
            exit;
        }
        /* 完成 */
        elseif (isset($_POST['finish']))
        {
            ...
            {
                /* 跳转到订单详情 */
                ecs_header("Location: car_order.php?act=info&order_id=" . $order_id . "\n");
                exit;
            }
        }
    }
    ...
    elseif ('consignee' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            ...
            {
                /* 存在实体商品，去配送方式 */
                ecs_header("Location: car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
                exit;
            }
            else
            {
                /* 不存在实体商品，去支付方式 */
                ecs_header("Location: car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=payment\n");
                exit;
            }
        }
        elseif (isset($_POST['finish']))
        {
            ...
            if ('edit' == $step_act && exist_real_goods($order_id))
            {
                ...
                if (!$exist)
                {
                    ...
                    $links[] = array('text' => $_LANG['step']['shipping'], 'href' => 'car_order.php?act=edit&order_id=' . $order_id . '&step=shipping');
            ...
            if ($agency_changed)
            {
                ecs_header("Location: car_order.php?act=list\n");
            }
            else
            {
                ecs_header("Location: car_order.php?act=info&order_id=" . $order_id . "\n");
        ...
        elseif (isset($_POST['finish']))
        {
            ...
            if ('edit' == $step_act && $shipping['support_cod'] == 0)
            {
                ...
                if ($payment['is_cod'] == 1)
                {
                    ...
                    $links[]    = array('text' => $_LANG['step']['payment'], 'href' => 'car_order.php?act=' . $step_act . '&order_id=' . $order_id . '&step=payment');
                }
            }
    ...
    elseif ('payment' == $step)
    {
        ...
        {
            /* 下一步 */
            ecs_header("Location: car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=other\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
    ...
    elseif ('other' == $step)
    {
        ...
        {
            /* 下一步 */
            ecs_header("Location: car_order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=money\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    /* 取得满足条件的订单 */
    $sql = "SELECT o.order_sn, u.uname " .
            "FROM " . $ecs->table('order_info') . " AS o " .
                "LEFT JOIN " . $sns->table('user') . " AS u ON o.user_id = u.uid " .
            "WHERE o.user_id > 0 " .
...
elseif ($_REQUEST['act'] == 'edit_templates')
{
    ...
    /* 提示信息 */
    $link[] = array('text' => $_LANG['back_list'], 'href'=>'car_order.php?act=list');
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    //     print_r(excel_output($_POST['order_id']));
    // }
    /* 指派 */
    ...
    //         /* 返回 */
    //         sys_msg($_LANG['order_removed'], 0, array(array('href'=>'car_order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])));
    //     }
    // }
    ...
    if (($require_note && $action_note == '') || isset($show_invoice_no) || isset($show_refund))
    {
        ...
        assign_query_info();
        $smarty->display('car_order_operate.htm');
    }
    else
    {
        /* 直接处理 */
        ...
        {
            /* 一个订单 */
            ecs_header("Location: car_order.php?act=operate_post&order_id=" . $order_id .
        ...
        {
            /* 多个订单 */
            ecs_header("Location: car_order.php?act=batch_operate_post&order_id=" . $order_id .
                    "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
            exit;
        }
    }
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    if ('cancel' == $operation)
    {
        ...
        {
            $sql = "SELECT * FROM " . $ecs->table('car_order') .
            ...
            if($order)
            {
                ...
                update_order($order_id, array('order_stat' => OS_CANCELED, 'to_buyer' => $cancel_note));

                /* 记录log */
                order_action($order['order_no'], OS_CANCELED, $order['shipping_status'], PS_UNPAYED, $action_note);
    ...
    /**/
    elseif ('excel' == $operation) {
        excel_output($_GET['order_id']);
        die();
    }
...
//    $action_note = $_REQUEST['action_note'];
    ...
    if(empty($sn_not_list))
    {
        ...
        $msg = $sn_list;
        $links[] = array('text' => $_LANG['return_list'], 'href' => 'car_order.php?act=list&' . list_link_postfix());
        sys_msg($msg, 0, $links);
    }
    else
    {
        ...
        $sql = "SELECT * FROM " . $ecs->table('car_order') .
                " WHERE order_no " . db_create_in($sn_not_list);
        $res = $db->query($sql);
        while($row = $db->fetchRow($res))
        {
        ...
        /* 显示模板 */
        assign_query_info();
        $smarty->display('car_order_operate_info.htm');
    }
}
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    /* modify by matao 2013.4.18
        订单状态：3：完成；4：取消;
         ...
         $arr = array('order_stat'  => 3); 
    */
         $arr = array('order_stat' => OS_CANCELED);
    ...
    elseif ('excel' == $operation)
    {
        excel_output($_REQUEST['order_id']);
        die();
    }
    /* 设为无效 */
    ...
    /* 操作成功 */
    $links[] = array('text' => $_LANG['order_info'], 'href' => 'car_order.php?act=info&order_id=' . $order_id);
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    {
        $url = 'car_order.php?act=query&' . str_replace('act=remove_order', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT uid, uname FROM " . $GLOBALS['sns']->table('user') .
                " WHERE uid LIKE '%" . mysql_like_quote($id_name) . "%'" .
                " OR uname LIKE '%" . mysql_like_quote($id_name) . "%'" .
                " LIMIT 20";
        ...
        {
             $result['userlist'][] = array('uid' => $row['uid'], 'uname' => $row['uname']);
        }
    }
    else
    {
...
function handle_order_money_change($order, &$msgs, &$links)
{
    ...
    if ($order['pay_status'] == PS_PAYED || $order['pay_status'] == PS_PAYING)
    {
        ...
        if ($money_dues > 0)
        {
            ...
            $links[]    = array('text' => $GLOBALS['_LANG']['order_info'], 'href' => 'car_order.php?act=info&order_id=' . $order_id);
        }
        elseif ($money_dues < 0)
        {
            $anonymous  = $order['user_id'] > 0 ? 0 : 1;
            $msgs[]     = $GLOBALS['_LANG']['amount_decrease'];
            $links[]    = array('text' => $GLOBALS['_LANG']['refund'], 'href' => 'car_order.php?act=process&func=load_refund&anonymous=' .
 ...
 * @return void
 */
function order_list($param_order_id='')
{
    $result = get_filter();
    if ($result === false)
    {
        /* 过滤信息 订单号,订单状态(新预定,执行中,已结束, 已取消,)转移状态(被转移,转移后),
        车辆昵称,车牌号,车群组,城市,站 点
        ,会员群组,会员手机号码,会员姓名,时间(包括:订车时间、实际取车时 间、实际还车时间(精确到天)、用车时间),支付方式*/
        ...
        $filter['car_id'] = empty($_REQUEST['car_id']) ? '' : intval($_REQUEST['car_id']);
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND o.order_no LIKE '%" . mysql_like_quote($filter['order_sn']) . "%'";
        }
        if ($filter['email'])
        {
        ...
        if ($filter['user_name'])
        {
            $where .= " AND u.true_name LIKE '%" . mysql_like_quote($filter['user_name']) . "%'";
        }
        ...
        {
            $where .= " AND o.order_stat = $filter[composite_status]";
        }
        if ($filter['car_id']>0)
        {
            $where .= " AND o.car_id = $filter[car_id]";
        }
        // //综合状态
        ...
        //         }
        // }
//订单号,订单状态,转移状态,订车时间,开始时间, 结束时间,订单时长,实际取车时间,实际还车时间,实际时长,会员姓名,
        //会员手机号码,车辆昵称,车牌号码,城市,站点,支付方式,开始里程,结束里 程,行驶里程,车辆使用费,附加费(超时费、退订费、改订费等)、使用奖章
        if ($param_order_id) 
        {
            $where .= " AND o.order_id = '$param_order_id' ";
        }
        ...
        if ($filter['user_name'])
        {
            $sql =  " SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('car_order') . " AS o ".
                    " LEFT JOIN ".$GLOBALS['sns']->table('user') . " AS u " . 
                    " ON o.user_id = u.uid ".
                    $where;
        }
        else
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('car_order') . " AS o ". $where;
        }
        ...
        /* 查询 */
        //订单号,订单状态,转移状态,订车时间,开始时间, 结束时间,订单时长,实际取车时间,实际还车时间,
        //实际时长,会员姓名,会 员手机号码,车辆昵称,车牌号码,城市,站点,支付方式,
        //开始里程,结束里 程,行驶里程,车辆使用费,附加费(超时费、退订费、改订费等)、使用奖章


        /*modify by matao 2013.4.22 
          完成公里数应给等与脉冲数除以该车里程因子 c.mile_factor 故此：多选一条c.mile_factor 字段。*/
        /*
        $sql =  " SELECT o.use_car_password,o.order_id, u.email,o.car_id,u.uid,u.true_name,o.order_no, o.order_stat,o.is_shift_stat,(o.end_mileage-o.start_mileage) as all_miles,
                        o.add_time, o.order_start_time, o.order_end_time,(o.order_end_time-order_start_time) as org_time_count, o.real_start_time,o.real_end_time,(o.real_end_time-real_start_time) as real_time_count," .
                    " u.uname, u.phone, c.name as car_name, c.number, r.region_name, s.name, " .
                    " o.start_mileage,o.end_mileage,(o.end_mileage-o.start_mileage) as mile_count,o.order_org_cost,o.order_real_cost,o.extra_cost,o.medal_id".
                ...
                " LEFT JOIN " .$GLOBALS['ecs']->table('station'). " AS s ON s.id=c.station ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('region'). " AS r ON r.region_id=s.city ".     
                $where .
                " ORDER BY $filter[sort_by] $filter[sort_order] ".
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
        */
        $sql =  " SELECT o.modify_stat,o.use_car_password,o.order_id, u.email,o.car_id,u.uid,u.true_name,o.order_no, o.order_stat,o.is_shift_stat,(o.end_mileage-o.start_mileage) as all_miles,
                        o.add_time, o.order_start_time, o.order_end_time,(o.order_end_time-order_start_time) as org_time_count, o.real_start_time,o.real_end_time,(o.real_end_time-real_start_time) as real_time_count," .
                    " u.uname, u.phone, c.name as car_name, c.number, r.region_name, s.name, " .
                    " o.start_mileage,o.end_mileage,(o.end_mileage-o.start_mileage) as mile_count,c.mile_factor,o.order_org_cost,o.order_real_cost,o.extra_cost,o.medal_id".
                " FROM " . $GLOBALS['ecs']->table('car_order') . " AS o " .
                " LEFT JOIN " .$GLOBALS['sns']->table('user'). " AS u ON u.uid=o.user_id ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('car'). " AS c ON o.car_id=c.id ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('station'). " AS s ON s.id=c.station ".
        ...
        foreach (array('order_no', 'order_stat', 'email', 'address', 'zipcode', 'tel', 'uname') AS $val)
        {
    ...
    {
        $fm_data[$key]=$value;
        $fm_data[$key]['add_time']= date('Y-m-d H:i',$value['add_time']);
        $fm_data[$key]['order_start_time']= date('Y-m-d H:i',$value['order_start_time']);
        $fm_data[$key]['order_end_time']= date('Y-m-d H:i',$value['order_end_time']);
        $fm_data[$key]['real_start_time']= date('Y-m-d H:i',$value['real_start_time']);
        $fm_data[$key]['real_end_time']= date('Y-m-d H:i',$value['real_end_time']);
        $fm_data[$key]['org_time_count']= number_format($value['org_time_count']/3600,2);

        /*modify by matao 2013.4.22 完成公里数的计算：*/

        $fm_data[$key]['all_miles']= number_format($value['all_miles']/$value['mile_factor'],1);
        $fm_data[$key]['start_mileage']= number_format($value['start_mileage']/$value['mile_factor'],1);
        $fm_data[$key]['end_mileage']= number_format($value['end_mileage']/$value['mile_factor'],1);
    ...
    /* modify by matao 2013.4.18
       状态与数据库中不匹配
        ...
        switch ($value['order_stat']) 
        {
             case 0:
                 $fm_data[$key]['order_stat'] = '新预定';
                 break;
             case 1:
                 $fm_data[$key]['order_stat'] = '执行中';
                 break;
             case 2:
                 $fm_data[$key]['order_stat'] = '已结束';
                 break;
             case 3:
                 $fm_data[$key]['order_stat'] = '取消';
                 break;
        }
        */
        ...
        {
             case 0:
                 $fm_data[$key]['order_stat'] = '未支付';
                 break;
             case 1:
                 $fm_data[$key]['order_stat'] = '已预订';
                 break;
             case 2:
                 $fm_data[$key]['order_stat'] = '执行中';
                 break;
             case 3:
                 $fm_data[$key]['order_stat'] = '已完成';
                 break;
             case 4:
                 $fm_data[$key]['order_stat'] = '已取消';
                 break;
             case 5:
                 $fm_data[$key]['order_stat'] = '支付超时';
                 break;
             case 6:
                 $fm_data[$key]['order_stat'] = '还车超时';
                 break;
             case 8:
                 $fm_data[$key]['order_stat'] = '已锁门';
                 break;
             case 9:
                 $fm_data[$key]['order_stat'] = '已结算';
                 break;    
        }

    /* modify by matao 2013.4.18*/

         $fm_data[$key]['is_shift_stat'] = $value['is_shift_stat']==0 ? '未转移':'已转移';
    }
    //print_r($fm_data);
    // foreach ($row AS $key => $value)
    // {
    ...
    //     }
    // }
    $arr = array('orders' => $fm_data, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
...
{
    return 'http://' . $_SERVER['HTTP_HOST'] . str_replace('/' . ADMIN_PATH . '/car_order.php', '', PHP_SELF);

}

function get_car_list($car_id)
{
    if ($car_id) 
    {
        $sql =  " SELECT c.*,s.name as station_name ,g.car_group_name,g.car_group_id ".
                " FROM ".$GLOBALS['ecs']->table('car')." c ".
                " LEFT JOIN ".$GLOBALS['ecs']->table('car_group')." g ".
                " ON g.car_group_id = c.group".
                " LEFT JOIN ".$GLOBALS['ecs']->table('station')." s ".
                " ON s.id = c.station ".
                " WHERE c.id=".$car_id.
...
function excel_output($order_id)
{
    /*检查order_id是否含有逗号,有则表示有多个id*/
    $where = " WHERE 0 "; 
    if(strpos($order_id,','))
    {
        $arr_order_id = explode(',',$order_id);
        foreach ($arr_order_id as $v) 
        {
            $where .= " OR o.order_id = '$v' ";
        }
    }
    else
    {
        $where .= " OR o.order_id = '$order_id' ";
    }

    $sql =  " SELECT o.order_no, o.order_stat,o.is_shift_stat,
                        o.add_time, o.order_start_time, o.order_end_time,(o.order_end_time-order_start_time) as org_time_count, o.real_start_time,o.real_end_time,(o.real_end_time-real_start_time) as real_time_count," .
                    " u.uname, u.phone, c.name as car_name, c.number, r.region_name, s.name, " .
                    " o.start_mileage,o.end_mileage,(o.end_mileage-o.start_mileage) as mile_count,o.order_org_cost,o.order_real_cost,o.extra_cost,o.medal_id".
                " FROM " . $GLOBALS['ecs']->table('car_order') . " AS o " .
    ...
    /*title*/
    $title = "<table width='200' border='1'> 
    <tr>  
    <td><strong>订单号</strong></td> 
    <td><strong>订单状态</strong></td>
    <td><strong>订车时间</strong></td>
    <td><strong>开始时间</strong></td>
    <td><strong>结束时间</strong></td>
    <td><strong>订单时长</strong></td>
    <td><strong>会员姓名</strong></td>
    <td><strong>会员手机号码</strong></td>
    <td><strong>车辆昵称</strong></td>
    <td><strong>车辆实际费用</strong></td>
    </tr>";
    echo $title;
    foreach ($fm_data as $v) 
    {
        ...
        echo " <td>".$v['order_stat']."</td>";
        echo " <td>".$v['add_time']."</td>";
        echo " <td>".$v['order_start_time']."</td>";
        echo " <td>".$v['order_end_time']."</td>";
        echo " <td>".$v['org_time_count']."</td>";
        echo " <td>".$v['uname']."</td>";
        echo " <td>".$v['phone']."</td>";
        echo " <td>".$v['name']."</td>";
        echo " <td>".$v['order_no']."</td>";

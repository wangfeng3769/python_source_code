...
class WaterMark extends Think
{
	...
	static public function iswater($filename,$savename=null){
		//读取默认配置
		$default['attach_watermark_closed'] = 0; // 是否开启水印功能，默认关闭
		$default['attach_watermark_minwidth'] = 300; // 添加水印最小宽度，默认300px
		$default['attach_watermark_minheight'] = 300; // 添加水印最小高度，默认300px
		$default['attach_watermark_type'] = 'img'; // 水印类型，默认图片水印
		$default['attach_watermark_txt'] = ''; // 文本水印内容
		...
		$default['attach_watermark_fontsize'] = 12; // 字号=体大小
		$default['attach_watermark_pct'] = 80; // 水印透明度，0-100默认80
		$default['attach_watermark_quality'] = 100; // 水印质量，0-100默认90
		$default['attach_watermark_pos'] = 9; // 水印位置，默认右下角
		//读取系统后台配置
	...
	static public function water($source, $conf, $savename=null) {
		...
		if($conf['attach_watermark_type'] == 'txt' && file_exists($conf['attach_watermark_font']))
		{
			...
			//可以加两层就是加一个阴影效果
			imagettftext ( $sImage, $fontSize, 0, $posX , $posY - 30, imagecolorallocate($sImage, 0, 0, 0), $fontType, $conf['attach_watermark_txt']);
			//imagettftext ( $sImage, $fontSize, 0, $posX -1 , $posY - 30 -1, imagecolorallocate($sImage, 255, 255, 255), $fontType, $conf['attach_watermark_txt']);
		}

 ...
 * ============================================================================
 * $Author: liuhui $
 * $Id: coupon.php 17063 2010-03-25 06:35:46Z liuhui $
*/
...
{
    $smarty->assign('ur_here',     $_LANG['bonus_type']);
    $smarty->assign('action_link', array('text' => $_LANG['coupontype_add'], 'href' => 'coupon.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('coupon_type.htm');
}
...
if ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('coupon_type.htm'), '',
...
{
    check_authz_json('coupon_manage');
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'coupon.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',      $_LANG['coupontype_add']);
    $smarty->assign('action_link',  array('href' => 'coupon.php?act=list', 'text' => $_LANG['05_coupon_list']));
    ...
    assign_query_info();
    $smarty->display('coupon_type_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $link[0]['href'] = 'coupon.php?act=add';
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('action_link', array('href' => 'coupon.php?act=list&' . list_link_postfix(), 'text' => $_LANG['05_coupon_list']));
...
if ($_REQUEST['act'] == 'update')
{
   ...
   /* 提示信息 */
   $link[] = array('text' => $_LANG['back_list'], 'href' => 'coupon.php?act=list&' . list_link_postfix());
...
if ($_REQUEST['act'] == 'send')
{
    ...
    if ($_REQUEST['send_by'] == SEND_BY_USER)
    {
        ...
        $smarty->display('coupon_by_user.htm');
    }
    /*废除按商品发红包~因此废弃以下*/
    ...
    elseif ($_REQUEST['send_by'] == SEND_BY_PRINT)
    {
        ...
        $smarty->display('coupon_by_print.htm');
    }
}
...
if ($_REQUEST['act'] == 'send_by_user')
{
    ...
    {
        /*  */
        $href = "coupon.php?act=send_by_user&start=" . ($start+$limit) . "&limit=$limit&id=$_REQUEST[id]&";
    ...
    $link[] = array('text' => $_LANG['back_list'], 'href' => 'coupon.php?act=list');
...
if ($_REQUEST['act'] == 'send_mail')
{
    ...
    $link[0]['href'] = 'coupon.php?act=bonus_list&bonus_type=' . $bonus['bonus_type_id'];
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    /* 提示信息 */
    $link[0]['text'] = $_LANG['back_bonus_list'];
    $link[0]['href'] = 'coupon.php?act=bonus_list&bonus_type=' . $bonus_typeid;
...
elseif ($_REQUEST['act'] == 'remove_bonus')
{
    ...
    $url = 'coupon.php?act=query_bonus&' . str_replace('act=remove_bonus', '', $_SERVER['QUERY_STRING']);
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        if (isset($_POST['drop']))
        {
            ...
            $link[] = array('text' => $_LANG['back_bonus_list'],
                'href' => 'coupon.php?act=bonus_list&bonus_type='. $bonus_type_id);
...
{
    /* 获得所有红包类型的发放数量 */
    $sql = "SELECT b.bonus_type_id, COUNT(1) AS sent_count".
            " FROM " .$GLOBALS['ecs']->table('user_bonus') .' b '. 
            " LEFT JOIN ".$GLOBALS['ecs']->table('bonus_type') .' t '.
            " ON b.bonus_type_id=t.type_id".
            " WHERE t.send_type=".SEND_BY_PRINT.
            " GROUP BY b.bonus_type_id";
    ...
    /* 获得所有红包类型的发放数量 */
    $sql = "SELECT b.bonus_type_id, COUNT(1) AS used_count".
            " FROM " .$GLOBALS['ecs']->table('user_bonus') .' b '. 
            " LEFT JOIN ".$GLOBALS['ecs']->table('bonus_type') .' t '.
            " ON b.bonus_type_id=t.type_id".
            " WHERE b.used_time > 0".
            " AND  t.send_type=".SEND_BY_PRINT.
            " GROUP BY b.bonus_type_id";
...
function get_bonus_list()
{
    ...
    $sql = "SELECT ub.*, u.user_name, u.email, o.order_no, bt.type_name ".
          ...
          " LEFT JOIN " .$GLOBALS['ecs']->table('car_order'). " AS o ON o.order_id=ub.order_id $where ".
          " ORDER BY ".$filter['sort_by']." ".$filter['sort_order'].

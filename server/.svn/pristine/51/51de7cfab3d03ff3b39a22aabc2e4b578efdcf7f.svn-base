...
function assignRegion()
{
	...
	{
		$sql = "select c.*,r.parent_id from ".$ecs->table("admin_column")." c
				left join ".$ecs->table("region")." r on c.city=r.region_id
				where c.admin_id=".$_SESSION['admin_id'] ;
		$allow = $db->getAll( $sql ) ;

		$allowCity = array() ;
		foreach( $allow as $k=>$v )
		{
			array_push( $allowCity, $v['city']."" ) ;
		}
		$allowCity = array_filter( $allowCity ) ;
		
		$cityWhere = " and ( region_id in (".join( ",", $allowCity).") or parent_id in (".join( ",", $allowCity).") )" ;

		$allowProvince = array() ;
		foreach( $allow as $k=>$v )
		{
			array_push( $allowProvince, $v['parent_id']."" ) ;
		}
		$allowProvince = array_filter( $allowProvince ) ;
		//print_r($allowProvince);
		$provinceWhere = " and ( region_id in (".join( ",", $allowProvince ).") or parent_id in (".join( ",", $allowProvince).") )" ;
	}

	$sql = "select * from ".$ecs->table("region")." where region_type=0 " . $provinceWhere." order by region_id" ;

	$province = $db->getAll( $sql ) ;

	$smarty->assign( "province", $province ) ;

	$sql = "select * from ".$ecs->table("region")." where region_type=2 ".$cityWhere." order by region_id" ; 
	$city = $db->getAll( $sql ) ;
	...
	if( $_GET['city'] )
	{
		$sql = "select parent_id from ".$ecs->table("region")." where region_id=".$_GET['city'] ;
		$province_id = $db->getOne($sql) ;

		$smarty->assign( "province_id", $province_id ) ;
		$smarty->assign( "city_id", $_GET['city'] ) ;
	}
}


function getRequestParam($p, $k)
{
	$v = str_replace("'","''",$p[$k]) ;
	return $v ;
}
...
{
	$POST[$k] = getRequestParam($_POST, $k) ;
}

$GET = array();
foreach($_GET as $k=>$v)
{
	$GET[$k] = getRequestParam($_GET, $k) ;
}


function alipayFreezeUrl($order_no)
{
	global $ALIPAY;
	$parameter = array(
		'service'           => "bank2freeze",
		...
		'buyer_email'      => 'zhou.tack@gmail.com',
		'amount' => '1.00',
		'op_type' => '0008',   //业务保证金,
		'order_from' => '100002'
	);

	return formatAlipayUrl($parameter);
}
function formatAlipayUrl($parameter)
{
	global $ALIPAY;
	ksort($parameter);
	reset($parameter);
	...
	$sign  = substr( $sign, 0, -1). $ALIPAY['key'] ;

...
//初始化ecshop架构
define('IN_ECS', true);
include_once('../../cp/includes/init.php');
...
//die(ROOT_PATH);
//sys_msg('成功加载ecshop',1,array(),false);
// $sql =	" SELECT * FROM ".$sns->table('user');
...
<body>
<h1>QQ互联集成PHP SDK - 首页<?php print_r($_SESSION);?></h1>
<div class="list-div">
<table>

<?php if(!isset($_SESSION["openid"])||empty($_SESSION["openid"])||empty($_SESSION["access_token"])||!isset($_SESSION["access_token"])){?>
<tr>
  <td>点击此处进行QQ登录：<a href="login.php"><img src="images/qqlogin.png" border="0" /></a>登录后方可享受API体验</td>
</tr>
<?php 
}else{
?>
<tr>
  <td>QQ用户已经登录,<a href="logout.php">点此退出</a></td>
	
</tr>
<?php 
if(isset($aConfig["api"]["get_user_info"])&&($aConfig["api"]["get_user_info"]==1)){
?>
<tr>

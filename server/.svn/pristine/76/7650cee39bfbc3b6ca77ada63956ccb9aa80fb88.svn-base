...
//
//		    if ($sync) {
//        		$from_data = array('app_type'=>'local_app', 'app_name'=>'group', 'gid'=>$save['gid'], 'title'=>$this->groupinfo['name'], 'url'=>U('group/Group/index', array('gid'=>$save['gid'])));
//        		$myweibo_id = D('Weibo', 'weibo')->publish( $this->mid , $data, 0 ,intval( $_POST['publish_type']) , h($_POST['publish_type_data']), true, serialize($from_data));
//        	}

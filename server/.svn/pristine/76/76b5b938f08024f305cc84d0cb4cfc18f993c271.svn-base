 ...
 * ============================================================================
 * $Author: wanglei $
 * $Id: credit_card.php 15013 2009-05-13 09:31:42Z wanglei $
 */
...
require_once( ROOT_PATH . "../moneyAccountManager/CreditAccountManager.php" );
...
if ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_link', array('href' => 'credit_card.php?act=add', 'text' => $_LANG['add_credit_card']));
	$smarty->assign('action_link2', array('href' => 'credit_card.php?act=createKeyPair', 'text' => $_LANG['credit_card_create_key_pair']));
...
else if( $_REQUEST['act'] == 'createKeyPair' ){
	$ccm = new CreditAccountManager();
	$ccm->createKeyPair();
}
...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	$url = 'credit_card.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'batch')
{
	...
	else
	{
		...
		if (isset($_POST['remove']))
		{
			...
			/* 清除缓存 */
			clear_cache_files();
			$links[] = array('href' => 'credit_card.php?act=list', 'text' => $_LANG['back_credit_card_list']);
...
elseif (in_array($_REQUEST['act'], array('add', 'edit')))
{
	...
	if ($_REQUEST['act'] == 'add')
	{
		...
		$smarty->assign('action_link', array('href' => 'credit_card.php?act=list', 'text' => $_LANG['credit_card_list']));
...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
	...
	{
		/* 提交值 */
		$a=substr(trim($_POST['credit_card_id']),0,4);
		$b=substr(trim($_POST['credit_card_id']), -4);
		$credit_card_id_merg=trim( $_POST['credit_card_id'] );//$a.'******'.$b ;
		...
		/* 判断信用卡是否重复 */
		$sql = "SELECT B.phone,C.is_processed,B.uid
                FROM ". $sns->table('user')." as B ,". $ecs->table('identity_approve')." as C 
                WHERE B.phone = '$phone'
                AND B.uid = C.user_id";
		$res = $db->getRow($sql);
		// if($res[is_processed]!=3&&$res[is_processed]!=7){
		// sys_msg($_LANG['credit_card_edit_processed']);
		// }
		//print_r($credit_card);exit();
		$db->autoExecute($ecs->table('credit_card'), $credit_card, 'INSERT');
		$credit_card['credit_card_id'] = $db->insert_id();

	}

	$ccm->saveToServer($credit_card['credit_card_id'] , $_POST);
	...
	/* 提示信息 */
	$links = array(array('href' => 'credit_card.php?act=add',  'text' => $_LANG['continue_add_credit_card']),
	array('href' => 'credit_card.php?act=list', 'text' => $_LANG['back_credit_card_list'])
	);
...
if ($_REQUEST['act'] == 'update')
{
	...
	$ccm->saveToServer($_POST['id'] , $_POST);

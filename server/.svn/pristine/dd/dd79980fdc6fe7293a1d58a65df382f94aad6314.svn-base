...
class UserAction extends AdministratorAction {
    ...
    public function user() {
    	$dao = D('User', 'home');
    	$res = $dao->getUserList('', true, true,true);
        $this->assign('uid',$uid);
    ...
    public function addUser() {
        ...
        $user['user_groups'] = model('UserGroup')->getUserGroupByMap('', 'user_group_id,title');
    ...
    public function doAddUser() {
        ...
        if (!$_POST['is_issue']&&!empty($_POST['card_number'])) {
           $this->error('必须通过实名认证才能添加会员卡');    
        }

        //检查电话号码是否已被注册
        $hasUsePhone=M('user')->where('phone='.$_POST['phone'])->getField('phone');
        if(!empty($hasUsePhone))
        {   
            $this->error('该手机号已被使用');
        }

        //检查会员号是否已使用
        $vipcardInfo=model('VipCard')->getVipCardByCardNumber($_POST['card_number']);
		...
		$_POST['is_init']   = '1';
        $_POST['validfor']  =strtotime($_POST['validfor']);
        $_POST['approve_id']=$_POST['is_issue'];
        ...
        //添加易多现金账户
        M('cash_account')->add(array('user_id'=>$uid ,'amount'=>0,'freeze_money'=>0));
        //添加实名认证通过的数据
        if ($_POST['is_issue']) {
            
            $approveStat=$_POST['user_type']==1 ? 7 : 3;
            M('cp_identity_approve')->add(array(
            ...
            if (!empty($_POST['card_number'])) {
                $vip_card_info = model('VipCard')->getVipCardByUid($uid);
                if ($_POST['card_number']!=$vip_card_info['card_number']) {
                    //更换会员卡
                    //删除原来的会员卡
                    model('VipCard')->deleteVipCardByUid($uid);
                    model('VipCard')->addCard($_POST['card_number'],$uid);
                }
            }
    ...
    public function editUser() {
        ...
        $cardInfo = model('VipCard')->getVipCardByUid($_GET['uid']);
        $card_number = $cardInfo['card_number'];
        $user['card_number']=$card_number;

        /**/
        $user['user_groups'] = model('UserGroup')->getUserGroupByMap('', 'user_group_id,title');
        $user['selected'] = model('UserGroup')->getUserGroupByUid($_GET['uid']);
        foreach ($user['user_groups'] as $k => $v) {
           $user['user_groups'][$k]['selected'] = $user['selected'][0]['user_group_id'] == $v['user_group_id'] ? 1 : 0;
        }
    ...
    public function doEditUser() {
        ...
        if (!empty($_POST['card_number'])) {
            $vip_card_info = model('VipCard')->getVipCardByUid($_POST['uid']);
            if ($_POST['card_number']!=$vip_card_info['card_number']) {
                //更换会员卡

                //删除原来的会员卡
                model('VipCard')->deleteVipCardByUid($_POST['uid']);
                model('VipCard')->addCard($_POST['card_number'],$_POST['uid']);
            }
        }
        /*检验表单vipcard是否为空,或者是否有修改*/
       

		//保存修改
		$key   			 = array('user_type','email','uname','sex','is_active','domain','phone','account_stat');
		$value 			 = array(intval($_POST['user_type']),$_POST['email'], escape(h(t($_POST['uname']))), intval($_POST['sex']), intval($_POST['is_active']),h($_POST['domain']),$_POST['phone'],intval($_POST['account_stat']));
		...
		$_LOG['type'] = '3';
		$data[] = '用户 - 用户管理 ';
		$data[] = M('user')->where($map)->field('uid,email,password,uname,domain,sex,is_active,phone,user_type')->find();
    ...
    public function doSearchUser() {
		...
		//组装搜索条件
    	$fields	= array('email','uid','sex','is_active','phone','location','account_stat');
    	$map	= array();
    	foreach($fields as $v)
        ...
        if ( isset($_POST['key_word']) && $_POST['key_word'] != '' ) {
            ...
            // $map['_query']   = " phone LIKE %'$_POST[key_word]'% &true_name LIKE %'$_POST[key_word]'% &_logic=or ";
            $map['true_name']   = array('exp', 'LIKE "%'.$_POST['key_word'].'%"');
            $map['phone']   = array('exp', 'LIKE "%'.$_POST['key_word'].'%"');
            $map['uname']   = array('exp', 'LIKE "%'.$_POST['key_word'].'%"');
            $map['_logic']="or";
        }

        //按订单状态,消费时间查询用户
        if ( isset($_POST['searchorder']) && $_POST['searchorder'] != '' ) {
            $startTime= empty($_POST['start']) ? strtotime(date("Y-m-d",time()-10*365*24*3600)) : strtotime($_POST['start']);
            $endTime=empty($_POST['end']) ? strtotime(date("Y-m-d", time()+10*365*24*3600))  : strtotime($_POST['end']);
            $endTime= $startTime==$endTime ? int($endTime)+24*3600 : $endTime;
            // $map['_query']   = " phone LIKE %'$_POST[key_word]'% &true_name LIKE %'$_POST[key_word]'% &_logic=or ";
            $map['add_time']   = array(array('gt',$startTime),array('lt',$endTime));
        ...
        // print_r($map);
    	$res = D('User', 'home')->getUserList($map, true, true,false,false,$isJoin);
    	$this->assign($res);
    	// $this->assign('type', 'searchUser'); 暂时不需要精确搜索
    ...
    {
        $_POST['gid'] = t($_POST['gid']);
        $res = model('UserGroup')->addPermit( $_POST['gid'] );
        if($res) echo 1;
        else     echo 0;
    }
    public function cancelPermit()
    {
        $_POST['gid'] = t($_POST['gid']);
        $res = model('UserGroup')->cancelPermit( $_POST['gid'] );
        if($res) echo 1;
        else     echo 0;
    }
    ...
    public function is_card_number($numbers='',$fields='*',$order='')
    {
        $res = model('VipCard')->getVipCardByCardNumber($numbers,$fields,$order);
        return $res;
    }

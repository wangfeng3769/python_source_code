...
class UserGroupModel extends Model {
    ...
    public function addUserToUserGroup($uids, $gids) {
    	self::updateGroupOnUser($uid,$gids);
    ...
    static public function updateGroupOnUser($uid,$gids)
    {
    	M('user')->where(array('uid'=>$_POST['uid']))->setField('user_group_id',$gids);
    }
    public function addPermit($gids)
    {
    	if (empty($gids)) return false;
		
    	$sql = "INSERT INTO `edo`.`edo_car_stop` (`group_id`) VALUES (".$gids.")";
		return $this->execute($sql);
    }
    public function cancelPermit($gids)
    {
    	if (empty($gids)) return false;
		
		$sql = "DELETE FROM `edo`.`edo_car_stop` WHERE `edo_car_stop`.`group_id` = ".$gids;
		return $this->execute($sql);
    }
}
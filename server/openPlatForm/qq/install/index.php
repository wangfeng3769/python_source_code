<?php
    ...
    if(isset($_POST)&&!empty($_POST)){
        if($_POST["session"]==0){
            ...
            mysql_connect($_POST["db"]["host"],$_POST["db"]["user"],$_POST["db"]["pass"]) or die("<script>alert('连接数据库失败，请重新配置。');location.href='index.php';</script>");
            mysql_select_db($_POST["db"]["name"]) or die("<script>alert('数据库不存在,请重新配置。');location.href='index.php';</script>");
            //执行建表任务
